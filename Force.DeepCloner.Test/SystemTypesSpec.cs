...
namespace Force.DeepCloner.Tests
{
	...
	public class SystemTypesSpec : BaseTest
	{
		...
		public void StandardTypes_Should_Be_Cloned()
		{
			...
			Assert.AreEqual(cloned.ToString(), ("test1"));
			var arr = new[] { 1, 2, 3 };
			var enumerator = arr.GetEnumerator();
			enumerator.MoveNext();
			var enumCloned = enumerator.DeepClone();
			enumerator.MoveNext();
			Assert.AreEqual(enumCloned.Current, (1));
		}
		...
		public void Type_With_Native_Resource_Should_Be_Cloned()
		{
			...
			try
			{
				...
				Assert.ThrowsException<ObjectDisposedException>(cloned.Flush);
		...
		public void Funcs_Should_Be_Cloned()
		{
			...
			closure[0] = "xxx";
			Assert.AreEqual(f(3), ("xxx3"));
			// we clone delegate together with a closure
			Assert.AreEqual(df(3), ("1233"));
			Assert.AreEqual(cf(3), ("xxx3"));
		}
		...
		public void Events_Should_Be_Cloned()
		{
			...
			eht.Event -= a1;
			eht.Event -= a2;
			Assert.AreEqual(eht.Call(1), (0)); // 0
			Assert.AreEqual(summ[0], (2)); // nothing to increment
			Assert.AreEqual(clone.Call(1), (2));
		}
